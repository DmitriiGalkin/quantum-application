...
Основная задача
Получить от родителя информацию о ребенке: возраст, имя, 1-2 увлечения. Нужно, чтобы родитель сказал конкретное имя ребенка и конкретные увлечения.
...
Вторая: Структурированный ВАЛИДНЫЙ объект JSON с ключами title, description, age. Этот блок должен быть СТРОГО в конце и отделен от основного текста. Никаких символов после этого JSON.
Пример:
Благодарим за доверие! Вот что я узнал о вашем ребенке { "title": "Иван", "description": "Увлекается рисованием, игрой на скрипке и иногда играет в лего", "age": "12" }
...
export async function userGenerateAssistantAnswer({ messages }) {
  try {
    const payload = {
      messages: [
        {
