...
import { CreateProject, type ProjectFullDto } from 'types';
...
export class ProjectService {
  ...
  static async findById(projectId: number, viewer?: Viewer): Promise<ProjectFullDto> {
    ...
    const placesForMeets = await Promise.all(meets.map(m => PlaceRepository.findById(m.placeId) as Promise<Place>));
